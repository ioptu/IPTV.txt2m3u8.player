...
- chrome直接播放的hls有时会出现只有声音，没有画面；一些非标准的hls目前chrome还是无法直接播放，比如链接中不含.m3u8关键字的hls类型视频，同时其响应头中content-type是text/plain。而使用其他播放器如potplayer则不存在这些问题，本扩展虽偶也会出现有声无画，但比chrome原生较少。
